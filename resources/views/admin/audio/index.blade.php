...
<span class="title-animation">Background Music Template</span>
@endsection
@section('content')
...
<div id="tabelusers"
    ...
    <div
        ...
        <form action="{{ route('audio.index') }}" method="GET">
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                    <div class="text-base text-black">{{ $d->file }}</div>
                </td>
                <td class="px-6 py-4 flex gap-x-2">
                    <button data-modal-target="main-modal" data-modal-toggle="main-modal"
                        ...
                        type="button"
                        onclick="openEditModal(edit=true, '{{ $d->id }}', '{{ $d->name }}', '{{ $d->file }}')">
                        Edit
                    </button>

                    <a href="/admin/audio/hapus/{{ $d->id }}"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5">
                <form id="formAudio" class="space-y-4" action="#" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="!mt-0">
                        <label for="name"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Judul Audio</label>
                        <input type="text" name="name" id="name" placeholder="e.g. Ed Sheeran - The Joker And The Queen"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Upload audio</label>
                        <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_help" id="file" name="file" type="file" accept=".mp3, .wav, .wma, .m4a">
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_help">MP3, WAV, WMA or M4A</p>
                    </div>
                    <button type="submit"
...
<script>
    edit = false;
    function openEditModal(edit, id, name, file) {
        if(edit==true){
            titleForm.innerHTML = "Edit Audio";
            document.getElementById('formAudio').action = "/admin/audio/update/" + id;
            document.getElementById('name').value = name;
            const fileInput = document.getElementById('file');
            if (file) {
                const existingFile = file.split('/').pop(); // Ambil nama file dari path jika ada
            } else {
            }
        } else {
            titleForm.innerHTML = "Tambah Audio";
            document.getElementById('formAudio').action = "{{ route('audio.store') }}";
            document.getElementById('bank').value = "";
            document.getElementById('file').value = "";
        }

        var existingHiddenInput = document.getElementById('formAudio').querySelector('input[name="_method"]');
        ...
        if (edit && !existingHiddenInput) {
            ...
            document.getElementById('formAudio').appendChild(hiddenInput);
