...
<span class="title-animation">Tambah Tamu Undangan</span>
@endsection
@section('content')

<form class="max-w-md mx-auto mt-12" method="post" action="{{ route('tamu.store') }}" enctype="multipart/form-data">
    @csrf
    @if ($errors->any())
    ...
    <!--<div class="relative z-0 w-full mb-5 group">
        ...
        <label for="position" class="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Posisi</label>
    </div>
